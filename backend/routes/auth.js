...
router.post(
  ...
  [
    body("username")
      .trim()
      .isLength({ min: 3 })
      .withMessage("Username deve essere almeno 3 caratteri"),
    ...
    body("nome")
      .trim()
      .isLength({ min: 3 })
      .withMessage("Nome deve contenere almeno un carattere"),
    body("cognome")
      .trim()
      .isLength({ min: 3 })
      .withMessage("Cognomeome deve contenere almeno un carattere"),
  ],
  async (req, res) => {
    try {
      // Valida input
      ...
      const [existingUsername] = await pool.query(
        "SELECT id_utente FROM utenti WHERE username = ?",
        [username],
      );
